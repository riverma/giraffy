// Giraffy's service worker. Hand-rolled, no build-time framework: the asset list and the
// cache name are injected by scripts/build-sw.mjs after each build (spec §5.16.2).
// Everything Giraffy needs is precached, so the app runs with the radio off, forever.
const VERSION = 'af71203b4026';
const CACHE = 'giraffy-' + VERSION;
const ASSETS = [
  "./",
  "./CNAME",
  "./assets/index-CQJgfviQ.css",
  "./assets/index-y1MOQvKk.js",
...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys()
      .then((keys) => Promise.all(keys.filter((k) => k !== CACHE && k.startsWith('giraffy-')).map((k) => caches.delete(k))))
