...
if npm audit --omit=dev --audit-level=high >/tmp/giraffy-audit.txt 2>&1; then
  echo "  no high or critical advisories"
else
  sed 's/^/  /' /tmp/giraffy-audit.txt | head -40
  fail=1
fi
