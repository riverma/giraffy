 ...
 */
export function isGuess(c: Pick<Card, 'from' | 'status'>, owner: string): boolean {
  return c.status === 'draft' && !!c.from && c.from !== owner;
}

export function statusLabel(c: Pick<Card, 'mine' | 'from' | 'to' | 'status'>): string {
  const o = otherName(c), s = c.status;
  if (!c.mine && s === 'draft') return 'Still a guess, in ' + c.from + "'s words";
  if (c.mine) {
    return ({
