...
import { needsAttention, otherName, STATE_NAMES, STATE_ORDER, statusDot, searchText } from './cards';
...
/** The predicate behind a filter id, or null for 'all'. */
export function filterTest(filter: string): ((c: Card) => boolean) | null {
  ...
  if (filter.startsWith('p:')) return (c) => otherName(c) === filter.slice(2);
...
export function filterGroups(cards: Card[], people: Person[]): FilterGroup[] {
  const item = (id: string, dot: string | null = null): FilterItem => {
    const t = filterTest(id);
  ...
  return [
    ...
    { name: 'Other', items: [item('attention'), item('entangled'), item('history')] }
  ].filter((g) => g.items.length);
}

export function applyListQuery(list: Card[], filter: string, query: string, sort: string): Card[] {
  const t = filterTest(filter);
