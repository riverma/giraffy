...
/**
 * The sentence a draft card carries. Their own wording once they have touched it; otherwise
 * the assembly of whatever is written so far, so a draft that gets shared half-finished
 * arrives saying something rather than nothing. An untouched draft stays empty, so it is
 * still known by its first line rather than by a sentence made entirely of ellipses.
 */
export function draftSummary(d: DraftState): string {
  if (d.summaryCustom) return d.summary;
  const written = d.observation.trim() || d.feelings.length || d.needs.length || d.request.trim();
  return written ? synthesize({ kind: d.kind, observation: d.observation, feelings: d.feelings, needs: d.needs, request: d.request }) : '';
}
