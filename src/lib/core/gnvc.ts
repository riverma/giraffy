...
/**
 * The permanent identifier, stamped into every card. It outlives any host, which is the
 * point of it, so it never changes even when the format moves.
 */
export const SPEC_URL = 'https://w3id.org/gnvc/1.0';
/** Where the format is published today, for links a person actually clicks. */
export const SPEC_PAGE = 'https://riverma.com/gnvc/';
 ...
 */
/**
 * A card that arrives by hand rarely arrives alone: earlier versions of Giraffy put a line
 * of prose above it, and a chat app may add a quoting header. Anything above the `gnvc:`
 * line that is not a YAML comment is somebody's note, not part of the card, so it is set
 * aside rather than handed to a parser that will refuse the whole thing because of it.
 */
export function cardText(text: string): string {
  const lines = text.split('\n');
  const at = lines.findIndex((l) => CARD_START.test(l) || l.trimStart().startsWith('#'));
  return at > 0 ? lines.slice(at).join('\n') : text;
}
/** A card's own first line is one of its top-level keys, or a comment above them. */
const CARD_START = /^(gnvc|kind|id|from|to|about|summary|observation|feelings|needs|requests|status|status_history|entangled_with|created|updated)[ \t]*:/;

export function parseCard(raw: string, owner: string): ParsedCard {
  try {
    return readCard(raw, owner);
  } catch (e) {
    // only now, with a straight read already refused, is it worth assuming the top of the
    // text was never part of the card
    const trimmed = cardText(raw);
    if (trimmed === raw) throw e;
    return readCard(trimmed, owner);
  }
}

function readCard(text: string, owner: string): ParsedCard {
