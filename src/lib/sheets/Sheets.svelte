<script lang="ts">
  ...
  import { otherName, statusLabel } from '$lib/core/cards';
  ...
  import { copyText, download, shareFile } from '$lib/share';
  ...
  async function doShare(): Promise<void> {
    if (!card) return;
    const ok = await shareFile(shareName, shareBody, 'A Giraffy card');
  ...
  const confirmDef = $derived.by(() => {
    ...
    if (k === 'revert') {
      ...
      body = S.confirm.deletePerson.body(app.data.cards.filter((c2) => otherName(c2) === name && c2.status !== 'draft').length);
...
{#if sheet?.kind === 'share' && card}
  <Sheet title="Share this card" onclose={close}>
    <div class="col" style="gap:12px;padding-bottom:8px">
      ...
      {#if app.data.preamble}
        <div class="ah-caption c-muted" style="font-style:italic">{S.share.preambleText}</div>
      {/if}
      <div class="yaml"><pre>{app.shareText(card)}</pre></div>
      <span class="ah-micro-caps c-muted">{shareName}</span>
      <div class="row wrap" style="gap:8px">
        <button class="btn sm" onclick={doShare}>Share…</button>
        <button class="btn ghost sm" onclick={copyShare}>Copy as text</button>
        <button class="btn ghost sm" onclick={() => { download(shareName, shareBody); app.toast(S.share.downloaded); }}>Download</button>
      </div>
    </div>
  </Sheet>
...
{:else if sheet?.kind === 'filters'}
  <Sheet title="Filter cards" onclose={close}>
    <div class="col scrolly" style="gap:16px;padding-bottom:8px;max-height:440px">
      ...
      {#each filterGroups(app.data.cards, app.data.people) as g (g.name)}
