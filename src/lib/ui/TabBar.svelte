<script lang="ts">
  ...
  $effect(() => {
    ...
    const set = (): void => {
      ...
      const room = Math.round(covered + 24);
      frame.style.setProperty('--scroll-tail', room + 'px');
      frame.style.setProperty('--tab-pad', room + 'px');
    };
    set();
    ...
    return () => {
      ...
      frame.style.removeProperty('--scroll-tail');
