...
export const S = {
  ...
  cardTo: {
    ...
    lead: 'The card stays exactly as you wrote it. Only who it is for changes.',
    shared: (name: string) => 'You shared this with ' + name + ' already. Pointing it at someone else sets it back to ready to share, since nobody new has seen it yet.',
    someoneElse: 'someone not in your people yet',
    same: 'It is already for them.',
    done: (name: string) => 'For ' + name + ' now. You can undo this.',
    note: (name: string) => 'Was for ' + name + '.',
    record: 'Changed who a card is for'
  },
  ...
  share: {
    ...
    preambleText: 'I took some time to put this into words with care, using Giraffy. You can simply read the card as it is, or open it at https://giraffy.riverma.com to reply from the heart.',
    exactly: 'Exactly this, and nothing above it: whoever receives it can paste it straight into their own Giraffy.',
    copied: 'Copied. Paste it into any chat, or into another Giraffy.',
    noteCopied: 'The note is copied. Send it in your own words if you would rather.',
  ...
  guess: {
    title: 'Imagine their card',
    lead: 'A first guess at what someone else might be feeling and needing, for them to correct.',
    voice: (name: string) => 'You are writing as ' + name + '. Every word of this is yours to be wrong about, and theirs to correct.',
    whose: 'Whose words are these?',
    whoseHint: 'One person. The card comes from them, and it is addressed to you.',
    still: (name: string) => 'Written by you, as ' + name + '. Not from ' + name + ' until ' + name + ' sends it.',
    send: (name: string) => 'Send it to ' + name,
    saved: (name: string) => 'Kept as a guess. Send it to ' + name + ' when you are ready, and they can put it right.',
    arrived: (name: string) => name + ' wrote this as you. Check it, change what is wrong, and send it back.',
    asName: (name: string) => 'as ' + name,
    recordStarted: 'Started a guess', recordSaved: 'Wrote a guess'
  },

  draft: {
    theirs: (name: string) => name + " wrote this and shared it while it was still a draft. Anything you add comes back to " + name + ".",
    sendBack: (name: string) => 'Send it back to ' + name,
    addTo: 'Add to it',
    mine: 'Still a draft. You can send it as it is and finish it later.'
  },
  ...
  importScreen: {
    toasts: {
      ...
      addedDraft: 'Waiting under Drafts. Put it right, then send it back.',
  ...
  update: {
    ready: 'A new version of Giraffy is ready. Your cards and needs are untouched.',
    button: 'Reload',
    updating: 'Reloading…'
  },
