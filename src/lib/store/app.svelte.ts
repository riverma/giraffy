...
import { draftCardFrom, draftContent, draftFace, draftFromCard, draftPeople, draftVoice, emptyIds, freshDraft, isEmptyDraft, linkedNeedId, sanitizeDraft } from '$lib/core/drafts';
...
export class App {
  ...
  /**
   * Hand over to the version waiting in the wings, then come back on it. The new worker
   * claims the page once it has skipped waiting, so this listens for that rather than
   * guessing at how long it takes, with a timeout in case a worker never claims at all.
   */
  async applyUpdate(): Promise<void> {
    ...
    // a worker that never takes over should not strand the page on the old version
  ...
  get received(): Card[] {
    // a guess is written as them but never sent by them, so it waits under Drafts instead
    return this.data.cards.filter((c) => !c.mine && c.status !== 'draft');
  }
  get drafts(): Card[] {
    return this.data.cards.filter((c) => c.status === 'draft').sort((a, b) => b.updated.localeCompare(a.updated));
  }
  ...
  pickFilter(id: string): void {
    ...
    // drafts live on neither side of Mine | Received, so the jump below does not apply to them
    if (next !== 'all' && t && next !== 'guess' && next !== 's:draft') {
  ...
  /**
   * Name somebody while a draft is being written, so they appear in People straight away
   * rather than only once the card is finished. Quiet on purpose: this rides along with the
   * draft's autosave, so it is not an undo step of its own, and `ensurePerson` already
   * returns the existing row for a name it knows and sends the owner to Myself.
   */
  namePerson(name: string): void {
    const nm = name.trim();
    if (!nm || nm === SELF_NAME || nm === this.data.owner) return;
    if (this.data.people.some((p) => p.name.toLowerCase() === nm.toLowerCase())) return;
    this.ensurePerson(nm);
    this.commit();
  }
  ...
  /**
   * A first guess at what someone else might say, written as them and addressed to you
   * (spec §5.7). It stays a draft until they have it and send it, because a card from them
   * that they never wrote would be a lie in your own records.
   */
  startGuess(personName: string | null = null, kind: Kind = 'request', returnTo: string | null = null, need: Need | null = null): void {
    const cm = freshComposer(kind);
    // empty means a guess with nobody picked yet; null would mean this is not a guess
    cm.asPerson = personName ?? '';
    cm.returnTo = returnTo;
    if (need) { cm.needs = [need.word]; cm.needIds = { [need.word]: need.id }; }
   ...
   * `asPerson !== null` (not `draftVoice`) is the test here: an empty string means a guess
   * with nobody picked yet, which is still a guess, not an ordinary draft.
   */
  private beginDraft(cm: Composer): void {
    this.record(cm.asPerson !== null ? S.guess.recordStarted : S.composer.recordStarted);
  ...
  resumeDraft(id: string): void {
    ...
    const state = c.draft ? sanitizeDraft(c.draft) : draftFromCard(c, this.data.owner);
    this.composer = { ...state, draftId: c.id, editId: null, returnTo: null };
    this.draftSeed = null;
  ...
  leaveComposer(): 'draft' | 'edit' | 'empty' | 'none' {
    ...
    if (isEmptyDraft(cm, seed)) {
      ...
      if (top?.label === S.composer.recordStarted || top?.label === S.guess.recordStarted) this.history = this.history.slice(0, -1);
      this.flush();
      return 'empty';
    }
  ...
  private syncDraft(cm: Composer): void {
    ...
    const face = draftFace(draft, this.data.owner);
    Object.assign(c, face, voice ? { mine: false, from: voice, to: this.data.owner } : { mine: true, from: this.data.owner },
      { draft, updated: now() });
  }
  ...
  saveComposer(): string | null {
    ...
    this.record(voice ? S.guess.recordSaved : d.editId ? S.composer.recordEdit : S.composer.recordNew);
    const at = now();
    const owner = this.data.owner;
    const persons = composerPeople(d).map((nm) => (nm === SELF_NAME ? owner : nm));
    if (voice) this.ensurePerson(voice);
    else for (const nm of composerPeople(d)) if (nm !== SELF_NAME && nm !== owner) this.ensurePerson(nm);
    ...
    // a guess is never finished here: it is theirs to finish, once they have it
    if (voice && d.draftId) {
      this.syncDraft(d);
      const c = this.card(d.draftId);
      if (c) Object.assign(c, { summary, updated: at });
      this.composer = null;
      this.draftSeed = null;
      this.setPrefs({ openDraft: null });
      this.commit();
      router.replace('/card/' + d.draftId, Math.max(1, router.depth));
      this.sheet = { kind: 'share', cardId: d.draftId };
      this.toast(S.guess.saved(voice));
      return d.draftId;
    }
  ...
  openShare(c: Card): void {
    // A draft can be sent at any stage: showing someone a half-written card is the point of
    // it, and sending leaves it a draft either way, so nothing is finished behind your back.
  ...
  /**
   * What travels as the card: the card, and nothing else. A note used to be pasted above it,
   * which made the text unreadable to the other end's import (spec §5.10). The note travels
   * beside the card now, not inside it.
   */
  shareBody(c: Card): string {
    return emitCard(c);
  }

  /** The line offered alongside a shared card, for whoever has never seen Giraffy. */
  shareNote(): string {
  ...
  /**
   * A draft written as you, under a spelling of your name this device does not use. Claiming
   * it makes it yours to finish: naming is a human act, so the app asks rather than guesses
   * (spec §5.10).
   */
  takeDraftAsMine(): void {
    const ir = this.importResult;
    if (ir?.type !== 'new' || ir.card.status !== 'draft') return;
    ir.card.from = this.data.owner;
    ir.card.mine = true;
    this.applyImport();
  }
  ...
  applyImport(): void {
    ...
    if (ir.type === 'merge') {
      ...
      if (i >= 0) {
        const merged = cloneCard($state.snapshot(ir.merged) as Card);
        // a guess that has come back in their words is no longer anybody's draft
        if (merged.status !== 'draft') delete merged.draft;
        this.data.cards[i] = merged;
      }
      this.clearImport();
      this.commit();
