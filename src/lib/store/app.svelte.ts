...
import { canReassign, cloneCard, isGuess, shareFilename } from '$lib/core/cards';
...
export class App {
  ...
  openShare(c: Card): void {
    // a guess is sent on purpose: it only becomes a real card once they have corrected it
    if (c.status === 'draft' && !isGuess(c, this.data.owner)) { this.toast(S.composer.finishFirst); return; }
