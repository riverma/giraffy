<script lang="ts">
  ...
  const saveLabel = $derived(voice ? 'This is my guess at ' + voice : cm?.editId ? 'Keep these changes' : people.length > 1 ? 'Write these ' + people.length + ' cards' : 'This is what I want to say');
  ...
  <div class="screen comp">
    ...
    {#if voice}
      <div style="padding:0 var(--gutter) 10px">
        <span class="ah-caption c-sec" style="font-style:italic">{S.guess.voice(voice)}</span>
      </div>
    {/if}
    <div class="scroll" style="padding:8px var(--gutter) 24px;gap:16px;--scroll-tail:0">
      {#if cm.step === 0}
        <div class="col" style="gap:20px;padding-top:8px">
          <div class="ah-heading-m c-head">{voice ? S.guess.title : cm.editId ? 'Editing a card' : 'A new card'}</div>
