<script lang="ts">
  ...
  const canNext = $derived.by(() => {
    if (!cm) return false;
    switch (cm.step) {
      case 0: return !!(cm.persons.length || cm.newPerson.trim());
  ...
  const saveLabel = $derived(cm?.editId ? 'Keep these changes' : people.length > 1 ? 'Write these ' + people.length + ' cards' : 'This is what I want to say');
  const starters = $derived(people[0] === SELF_NAME
  ...
  function next(): void {
    ...
    const n = seq[pos + 1];
    if (n === 5 && !cm.summaryCustom) cm.summary = synthesize({ kind: cm.kind, observation: cm.observation, feelings: cm.feelings, needs: cm.needs, request: cm.request });
    go(n);
  }
...
{#if cm}
  <div class="screen comp">
    ...
    <div class="scroll" style="padding:8px var(--gutter) 24px;gap:16px">
      {#if cm.step === 0}
        <div class="col" style="gap:20px;padding-top:8px">
          <div class="ah-heading-m c-head">{cm.editId ? 'Editing a card' : 'A new card'}</div>
      ...
      {:else if cm.step === 1}
        <div class="col" style="gap:14px;padding-top:8px">
          ...
          <div class="ah-pull-quote c-head">{cm.kind === 'gratitude'
      ...
      {:else if cm.step === 2}
        <div class="col" style="gap:14px;padding-top:8px">
          ...
          <div class="ah-pull-quote c-head">When that happened, what did you feel, in you?</div>
      ...
      {:else if cm.step === 3}
        <div class="col" style="gap:14px;padding-top:8px">
          ...
          <div class="ah-pull-quote c-head">{cm.kind === 'gratitude' ? 'Which of your needs were met? Try completing: I need… or I value…' : 'Which of your needs are not being met? Try completing: I need… or I value…'}</div>
          {#if Object.keys(app.data.needTiers).length}
              ...
              {#if cm.expandedCat === cat.name}
                <div class="row wrap" style="gap:7px;padding:2px 4px 8px">
                  {#each cat.needs as n (n.id)}
                    {@const cs = chipStyle(linkedNeedId(cm, n.word) === n.id, app.tierOf(n.id))}
      ...
      {:else if cm.step === 4}
        <div class="col" style="gap:14px;padding-top:8px">
          ...
          <div class="ah-pull-quote c-head">What would you like to ask? A request is a gift of clarity, not an obligation. The other person is free to say no.</div>
      ...
      {:else}
        <div class="col" style="gap:14px;padding-top:8px">
          ...
          <textarea class="ta synth" rows="8" bind:value={cm.summary} oninput={() => { cm.summaryCustom = true; touch(); }}></textarea>
          ...
          <button class="btn wide" onclick={() => app.saveComposer()}>{saveLabel}</button>
        </div>
      {/if}
    </div>
    <div class="foot">
      ...
      <div class="grow"></div>
      {#if cm.step !== 5}<button class="btn" disabled={!canNext} onclick={next}>Continue</button>{/if}
    </div>
  </div>
{/if}
