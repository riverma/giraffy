<script lang="ts">
  ...
  const preview = $derived.by(() => {
    ...
    if (ir.type === 'new') {
      const c = ir.card;
      const state = c.status === 'draft' ? 'draft' : 'received';
      return { name: c.from, kindLabel: c.kind + ' · from ' + c.from, dateStr: fmtDate(now()), summary: c.summary,
        dot: statusDot(state), statusLabel: statusLabel({ mine: c.mine, from: c.from, to: app.data.owner, status: state, history: c.history }),
        note: '', noteBy: '' };
    }
    if (ir.type === 'merge') {
  ...
  const mergeCaption = $derived(ir?.type === 'merge'
    ...
    : '');
  // a draft that arrives was written as somebody, by somebody else: the history says who
  const arriving = $derived(ir?.type === 'new' && ir.card.status === 'draft' ? ir.card : null);
  const guessedBy = $derived(arriving?.history.find((h) => h.state === 'draft')?.by ?? '');
...
<div class="screen over">
  ...
  <div class="scroll" style="padding-top:8px;gap:14px">
    ...
    {#if formatHelp}
      <div class="card sunk" style="padding:16px">
        <div class="col" style="gap:12px">
          ...
          {/if}
          <div style="align-self:flex-start"><a class="pill outline" href={SPEC_PAGE} target="_blank" rel="noopener">Read the full format ›</a></div>
        </div>
      </div>
    {/if}
    {#if restoring}
    ...
    {#if ir?.type === 'new' && arriving}
      {#if arriving.mine}
        <div class="ah-caption c-sec">{S.guess.arrived(guessedBy || 'Someone')}</div>
        <div class="row" style="gap:8px">
          <button class="btn" onclick={() => app.applyImport()}>Take it as my draft</button>
          <button class="btn ghost" onclick={() => (app.importResult = null)}>Not now</button>
        </div>
      {:else}
        <div class="ah-caption c-sec">This is written as <strong>{arriving.from}</strong>{guessedBy ? ', by ' + guessedBy : ''}. If that name is you under another spelling, take it as yours and it becomes your draft to finish.</div>
        <div class="row wrap" style="gap:8px">
          <button class="btn" onclick={() => app.takeDraftAsMine()}>That is me, take it</button>
          <button class="btn ghost" onclick={() => app.applyImport()}>Keep it as it is</button>
          <button class="btn ghost" onclick={() => (app.importResult = null)}>Not now</button>
        </div>
      {/if}
    {:else if ir?.type === 'new'}
