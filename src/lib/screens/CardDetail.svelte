<script lang="ts">
  ...
  import { canCelebrate, canEdit, canGive, canReassign, canRespond, canWithdraw, cardNeedIds, entryLabel, isGuess, isTheirDraft, otherName, statusDot, statusLabel } from '$lib/core/cards';
  ...
  /** Their own unfinished card, sent over for you to read or add to. */
  const theirDraft = $derived(!!d && isTheirDraft(d, app.owner));
    ...
    <div class="scroll" style="padding-top:4px;gap:18px">
      <div class="card" style="padding:22px">
        <div class="col" style="gap:14px">
          ...
          {:else if theirDraft}
            <span class="ah-caption c-muted">{S.draft.theirs(d.from)}</span>
          {:else if d.status === 'draft'}
            <span class="ah-caption c-muted">{S.draft.mine}</span>
          {/if}
        </div>
      </div>
        ...
        <div class="col" style="gap:14px">
          ...
          <div class="col" style="gap:9px">
            ...
            <div class="row wrap" style="gap:7px">
              {#each d.needs as w, i}
                <button class="pill" onclick={() => openNeed(i)}><span class="dot" style="background:{needIds[i] && !guessing ? tierDot(app.tierOf(needIds[i])) : 'var(--saffron-500)'}"></span>{w}</button>
              {/each}
            </div>
          </div>
      ...
      <div class="row wrap" style="gap:8px;padding-top:4px">
        {#if d.status === 'draft'}
          ...
          {:else if theirDraft}
            <button class="btn" onclick={() => app.openShare(d)}>{S.draft.sendBack(d.from)}</button>
            <button class="btn ghost" onclick={() => app.resumeDraft(d.id)}>{S.draft.addTo}</button>
          {:else}
            <button class="btn" onclick={() => app.resumeDraft(d.id)}>Continue writing</button>
            <button class="btn ghost" onclick={() => app.openShare(d)}>{S.composer.sendAsIs}</button>
          {/if}
