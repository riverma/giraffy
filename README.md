...
**A guess needs no field of its own.** "Imagine their card" writes a draft as someone else:
`from` is them, `to` is the owner, `mine: false`, `status: draft`. `isGuess()`
(`core/cards.ts`) is just `status === 'draft' && from !== owner`; nothing new in `Card`, and
nothing that has to be stripped before a share. The trick is that the same rule Giraffy
already used to sort an imported card into Mine or Received — `from` matched against the
owner's name — does the rest of the work for free: on the sender's device the card is a
guess in Drafts, and on the recipient's device the identical file lands as their own draft,
because `mine` is computed the same way there too.
...
**Asset filenames are stable, and must stay that way.** GitHub Pages serves `index.html`
with `max-age=600` and offers no way to change it, so for ten minutes after a deploy a
browser can still be running the previous page. Content-hashed filenames plus a deploy that
deletes the old ones means that page asks for a file which is gone, gets a 404, and shows
nothing: it happened in Safari on 2026-09-21. Cache busting is the service worker's job here,
and its cache name is a hash of file *contents*, so stable names cost nothing. `deploy.sh`
also carries forward any file the previous deploy had and the new one does not.
