...
// A deploy renames or drops a file and a browser is still holding the previous page: the
// script 404s and nothing mounts. That must not be a blank screen with no way out.
test('a page whose script will not load says so, rather than showing nothing', async ({ page }) => {
  await page.route('**/assets/*.js', (r) => r.fulfill({ status: 404, body: '' }));
  await page.goto('/');
  await page.waitForTimeout(800);

  const fallback = page.locator('#boot-fallback');
  await expect(fallback).toBeVisible();
  await expect(fallback).toContainText('could not finish loading');
  // and it says the words are safe, because that is the first thing anyone would fear
  await expect(fallback).toContainText('stored on this device');
  // the app never mounted, so there is no screen behind it
  expect(await page.locator('.screen').count()).toBe(0);
});

test('the boot message stands down once the app mounts', async ({ page }) => {
  await page.goto('/?demo');
