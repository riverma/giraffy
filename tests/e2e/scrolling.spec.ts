...
// scroll at all, but the last area must never be stuck underneath the bottom bar. These
// measure the outcome a thumb gets, not how the room happens to be reserved.
async function clearanceAtTheEnd(page: Page): Promise<{ clearance: number; covered: number }> {
  await page.evaluate(() => document.querySelector('.screen .scroll')!.scrollTo(0, 99999));
  await page.waitForTimeout(300);
  return page.evaluate(() => {
    ...
    return { clearance: Math.round(bar.top - last.bottom), covered: Math.round(frame.bottom - bar.top) };
  });
}
...
for (const [name, width, height] of PHONES) {
  test(`the last area is never stuck under the bottom bar on ${name}`, async ({ page }) => {
    ...
    // scrolled as far as it goes, the last area stands clear of the bar
    const { clearance } = await clearanceAtTheEnd(page);
    expect(clearance, 'clearance below the last area').toBeGreaterThanOrEqual(16);
  });
}
...
test('a bottom bar taller than anyone guessed still leaves room for the last area', async ({ page }) => {
  ...
  const { clearance, covered } = await clearanceAtTheEnd(page);
  expect(covered, 'the bar really did grow').toBeGreaterThan(130);
  expect(clearance, 'clearance below the last area').toBeGreaterThanOrEqual(16);
});
