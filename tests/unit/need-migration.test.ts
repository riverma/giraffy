...
describe('what the load path hands to the app', () => {
  it('stays an ordinary object, so the screen still follows it', () => {
    // Svelte does not proxy an object whose prototype is null: one of those in app.data
    // means marking a need changes the data and nothing on screen
    const d = data({
      needTiers: { 'peace/space': { tier: 'met', changed: '2026-07-01T00:00:00Z', was: null } },
      needNotes: { 'meaning/movement': 'still moving' }
    });
    const out = migrateNeedIds(d);
    expect(Object.getPrototypeOf(out.needTiers)).toBe(Object.prototype);
    expect(Object.getPrototypeOf(out.needNotes)).toBe(Object.prototype);
  });
});
