...
import { cardText, emitCard, parseCard, GnvcError } from '$lib/core/gnvc';
...
describe('a card that arrives with something above it', () => {
  const body = emitCard(cards[0]);
  const note = 'I took some time to put this into words with care, using Giraffy. You can simply\nread it below, or open it at https://giraffy.riverma.com to reply from the heart.';

  test('reads the card when a note was pasted above it, as older versions sent', () => {
    const { card } = parseCard(note + '\n\n' + body, owner);
    expect(card.summary).toBe(cards[0].summary);
    expect(card.to).toBe(cards[0].to);
  });

  test('reads the card through a chat app quoting header', () => {
    const { card } = parseCard('> Maya sent:\n> \nHere you go!\n\n' + body, owner);
    expect(card.summary).toBe(cards[0].summary);
  });

  test('keeps the comments a card ships with', () => {
    expect(cardText(body)).toBe(body);
    expect(body.startsWith('# A Giraffy card')).toBe(true);
  });

  test('leaves a card that starts with the card alone', () => {
    const bare = body.split('\n').filter((l) => !l.startsWith('#')).join('\n').trim();
    expect(cardText(bare)).toBe(bare);
  });

  test('still refuses something that is not a card at all', () => {
    expect(() => parseCard('just a message, no card here', owner)).toThrow(GnvcError);
  });
});
