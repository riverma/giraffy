...
describe('a draft', () => {
  ...
  it('can be sent half-written, and is still a draft afterwards', async () => {
    const app = await ready();
    app.startCompose('request');
    Object.assign(app.composer!, { persons: ['Robin'], observation: 'When the rota went up' });
    app.touchComposer();
    const card = app.card(app.composer!.draftId!)!;

    app.openShare(card);
    expect((app.sheet as { kind?: string } | null)?.kind).toBe('share');
    // sending does not finish it: no shared entry, still a draft, still in Drafts
    expect(app.card(card.id)?.status).toBe('draft');
    expect(app.card(card.id)?.history.map((h) => h.state)).toEqual(['draft']);
    expect(app.drafts.length).toBe(1);
    expect(app.mine.length).toBe(0);

    // and what travels carries the sentence so far, not an empty summary
    const text = app.shareBody(app.card(card.id)!);
    expect(text).toMatch(/^status: draft$/m);
    expect(text).toMatch(/the rota went up/);
  });
...
describe('imagining someone else\'s card', () => {
  ...
  it('can be sent, and says whose words it is in', async () => {
    const app = await ready();
    app.startGuess('Robin');
    ...
    // a draft of your own travels too, and says it is yours
    app.sheet = null;
    app.startCompose('request');
    ...
    app.openShare(plain);
    expect((app.sheet as { kind?: string } | null)?.kind).toBe('share');
    expect(app.shareBody(app.card(plain.id)!)).toMatch(/^from: Ash$/m);
  });
  ...
  it('comes back in their words, with your guess kept in the history', async () => {
    ...
    // A reply comes later than the guess, and timestamps are whole seconds, so a test that
    // runs in milliseconds has to say so. Derived from the card rather than written down: a
    // fixed date here passes until the day it goes by, and then fails every day after.
    back.updated = new Date(Date.parse(back.updated) + 86_400_000).toISOString().replace(/\.\d+Z$/, 'Z');
