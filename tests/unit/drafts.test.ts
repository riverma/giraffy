...
import { draftCardFrom, draftContent, draftFromCard, draftLabel, draftSummary, freshDraft, isEmptyDraft, linkedNeedId, sanitizeDraft } from '../../src/lib/core/drafts';
...
describe('the sentence a draft carries', () => {
  it('assembles from whatever is written, so a half-written card still says something', () => {
    const d = { ...freshDraft('request'), observation: 'the rota went up without me', feelings: ['weary'] };
    const s = draftSummary(d);
    expect(s).toContain('the rota went up without me');
    expect(s).toContain('weary');
    // the parts not yet written are visible as gaps rather than guessed at
    expect(s).toContain('…');
  });

  it('leaves an untouched draft with no sentence at all', () => {
    // otherwise every empty draft would be known by a line made only of ellipses
    expect(draftSummary(freshDraft('request'))).toBe('');
  });

  it('keeps their own wording once they have written it', () => {
    const d = { ...freshDraft('request'), observation: 'something', summary: 'My own words.', summaryCustom: true };
    expect(draftSummary(d)).toBe('My own words.');
  });

  it('reaches the card, so what gets shared is not an empty summary', () => {
    const d = { ...freshDraft('request'), persons: ['Robin'], observation: 'the rota went up', feelings: ['weary'] };
    const c = draftCardFrom(d, 'Maya', 'c1', '2026-09-22T10:00:00Z');
    expect(c.summary).toContain('the rota went up');
  });
});
